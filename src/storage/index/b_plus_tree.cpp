...
auto BPLUSTREE_TYPE::GetValue(const KeyType &key, std::vector<ValueType> *result, Transaction *transaction) -> bool {
  ...
  auto leaf_ptr = reinterpret_cast<IN_TREE_LEAF_PAGE_TYPE *>(page_ptr->GetData());
  RID temp_rid;
...
void BPLUSTREE_TYPE::StartNewTree(const KeyType &key, const ValueType &value) {
  ...
  auto leaf_ptr = reinterpret_cast<IN_TREE_LEAF_PAGE_TYPE *>(root_page_ptr->GetData());
...
auto BPLUSTREE_TYPE::InsertIntoLeaf(const KeyType &key, const ValueType &value, Transaction *transaction) -> bool {
  ...
  if (size == leaf_ptr->GetMaxSize()) {
    auto new_leaf_ptr = SplitPage<IN_TREE_LEAF_PAGE_TYPE>(leaf_ptr);
...
INDEX_TEMPLATE_ARGUMENTS
auto BPLUSTREE_TYPE::FindLeafPage(const KeyType &key, bool leftMost, int mode, Transaction *transaction) -> Page * {
  ...
  while (true) {
    ...
    auto internal_ptr = reinterpret_cast<IN_TREE_INTERNAL_PAGE_TYPE *>(tree_ptr);
...
INDEX_TEMPLATE_ARGUMENTS
template <typename N>
auto BPLUSTREE_TYPE::SplitPage(N *node) -> N * {
...
void BPLUSTREE_TYPE::InsertIntoParent(BPlusTreePage *old_node, const KeyType &key, BPlusTreePage *new_node,
  ...
  if (old_node->IsRootPage()) {  // Which means root has been split and need to construct a new root
    ...
    auto new_root_ptr = reinterpret_cast<IN_TREE_INTERNAL_PAGE_TYPE *>(new_root_page_ptr->GetData());
  ...
  auto parent_ptr = reinterpret_cast<IN_TREE_INTERNAL_PAGE_TYPE *>(parent_page_ptr->GetData());

  int new_size = parent_ptr->InsertNodeAfter(old_node->GetPageId(), key, new_node->GetPageId());

  if (new_size == parent_ptr->GetMaxSize()) {
    auto new_parent_ptr = SplitPage<IN_TREE_INTERNAL_PAGE_TYPE>(parent_ptr);

    parent_ptr->MoveHalfTo(new_parent_ptr, buffer_pool_manager_);
    InsertIntoParent(parent_ptr, new_parent_ptr->KeyAt(0), new_parent_ptr, transaction);
...
void BPLUSTREE_TYPE::Remove(const KeyType &key, Transaction *transaction) {
  ...
  auto leaf_ptr = reinterpret_cast<IN_TREE_LEAF_PAGE_TYPE *>(leaf_page_ptr->GetData());

  int index = leaf_ptr->KeyIndex(key, comparator_);
  if (comparator_(leaf_ptr->KeyAt(index), key) != 0) {
    return;
  }
...
auto BPLUSTREE_TYPE::AdjustRoot(BPlusTreePage *old_root_node) -> bool {
  ...
  if (old_root_node->IsLeafPage()) {
    ...
    auto old_root_internal_node = reinterpret_cast<IN_TREE_INTERNAL_PAGE_TYPE *>(old_root_node);
...
void BPLUSTREE_TYPE::Redistribute(N *neighbor_node, N *node, int index) {
  ...
  if (node->IsLeafPage()) {
    auto op_node = reinterpret_cast<IN_TREE_LEAF_PAGE_TYPE *>(node);
    auto op_neighbor_node = reinterpret_cast<IN_TREE_LEAF_PAGE_TYPE *>(neighbor_node);
    ...
    auto op_node = reinterpret_cast<IN_TREE_INTERNAL_PAGE_TYPE *>(node);
    auto op_neighbor_node = reinterpret_cast<IN_TREE_INTERNAL_PAGE_TYPE *>(neighbor_node);
...
auto BPLUSTREE_TYPE::Coalesce(N **neighbor_node, N **node,
  ...
  if (!neighbor_node || !node || !(*neighbor_node) || !(*node)) {
    return false;
  }
